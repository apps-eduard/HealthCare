...
- **DOCTOR:** own appointments + own availability; medical notes with tightened ownership (DR-6); appointment-linked patients (Model A, DR-5). No clinic administration. Authoritative Web MVP scope: `Docs/mvp-doctor-scope.md` (**approved** 2026-07-25). **In code:** `doctor_dashboard.read` (DR-1); `doctor_profile.read` / `doctor_profile.update` (DR-2); availability self-manage + queue/calendar doctor filter locked to self (DR-3); appointment API ownership + `appointments.create` removed from DOCTOR (DR-4). **Current code gaps until later phases:** clinic-wide patient search (DR-5), clinic-wide note read/amend (DR-6).
