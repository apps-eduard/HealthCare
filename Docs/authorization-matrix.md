...
- **PATIENT:** own profile/appointments + availability read + clinic register + authenticated clinic browse (`GET /api/v1/patients/me/clinics`); no staff ops. Authoritative Mobile MVP: `Docs/mvp-patient-scope.md` (**approved** 2026-07-25; **PM-0…PM-4 delivered**; PM-5…PM-8 pending). PM-1: foreign patient profile `404`; 2-hour cancel/reschedule cutoff (`appointment.patient_mutation_cutoff` / 409). PM-4: Patient clinic directory separate from staff `staff-management/clinics`. Remaining: booking/appointments mobile (PM-5+). No medical notes / staff ops / confirm / check-in.
