...
- **CLINIC_ADMIN:** clinic-scoped ops; `clinics.read` + `clinic_dashboard.read` + `clinic_profile.read` / `clinic_profile.update`; no organization clinic create/update/activate/deactivate; cannot assign ORG/PLATFORM admin. Authoritative Web MVP scope: `Docs/mvp-clinic-admin-scope.md` (**approved** 2026-07-24). **In code:** `clinic_dashboard.read`, `clinic_profile.read` / `clinic_profile.update`. **Approved, not yet coded:** `clinic_reports.read`, `clinic_audit_logs.read`. Do not grant `organization_*` dashboard/reports/audit/usage/profile or `medical_notes.*` via CLINIC_ADMIN alone.
