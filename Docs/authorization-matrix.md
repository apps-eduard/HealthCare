...
| `clinic_profile.update` | Clinic Admin (and PLATFORM_ADMIN with explicit clinic bypass) clinic profile settings update |
...
| `organization_profile.read` | Organization Admin (and PLATFORM_ADMIN with explicit tenant bypass) organization profile settings read (`/organization/settings`) |
| `organization_profile.update` | Organization Admin (and PLATFORM_ADMIN with explicit tenant bypass) organization profile settings update |
...
| `medical_notes.read` | Read authorized note summaries/detail (author + own appointment for Doctor) |
...
- **PLATFORM_ADMIN:** broad permissions including `organization_dashboard.read` / `organization_reports.read` / `organization_audit_logs.read` / `organization_usage.read` / `organization_profile.read` / `organization_profile.update` + `organizations.read` / `organizations.select`; **does not** auto-bypass tenants — requires `PlatformAdminBypass.Explicit`. Organization directory listing is a platform operation and does **not** grant clinic/resource access. No `medical_notes.*`. Staff Patient Directory (`/patients`, `/api/v1/staff/patients`) is intentional when `patients.search` / `patients.read` are granted; this is **not** Patient self-service. `patients.update_own_profile` remains in the PLATFORM_ADMIN catalog as historical breadth — it is **harmless** for escalation because `/api/v1/patients/me` still requires `PatientSelfScope` (PATIENT role + linked Patient). Optional future hygiene: remove `patients.update_own_profile` from PLATFORM_ADMIN only after confirming no downstream assumption depends on the catalog grant.
...
- **CLINIC_ADMIN:** clinic-scoped ops; `clinics.read` + `clinic_dashboard.read` + `clinic_profile.read` / `clinic_profile.update` + `clinic_reports.read` + `clinic_audit_logs.read`; no organization clinic create/update/activate/deactivate; cannot assign ORG/PLATFORM admin. Authoritative Web MVP scope: `Docs/mvp-clinic-admin-scope.md` (**approved** 2026-07-24; **CA-1–CA-10 complete**). **In code:** 29 operational permissions only. Do not grant `organization_*` dashboard/reports/audit/usage/profile or `medical_notes.*` via CLINIC_ADMIN alone.
- **DOCTOR:** own appointments + own availability; medical notes with author + own-appointment ownership; appointment-linked patients (Model A). No clinic administration. Authoritative Web MVP scope: `Docs/mvp-doctor-scope.md` (**approved** 2026-07-25; **DR-1–DR-7 + DR-9 + DR-10 complete**; DR-8 skipped). **In code:** `doctor_dashboard.read` (DR-1); `doctor_profile.read` / `doctor_profile.update` (DR-2); availability self-manage + queue/calendar doctor filter locked to self (DR-3); appointment API ownership + `appointments.create` removed from DOCTOR (DR-4); Model A patient search/detail (DR-5); medical-note ownership + appointment-detail UX (DR-6); clinical workflow / completion hardening (DR-7); cross-role negative matrix (DR-9); Playwright Doctor E2E pack (DR-10). Doctor reports skipped (DR-8).
...
- **PATIENT:** own profile/appointments + availability read + clinic register + authenticated clinic browse + mobile booking (`Requested`) + My Appointments cancel/reschedule (2-hour cutoff). Authoritative Mobile MVP: `Docs/mvp-patient-scope.md` (**approved** 2026-07-25; **PM-0…PM-8 delivered**; Patient Mobile MVP complete). PM-1: foreign patient profile `404`; 2-hour cancel/reschedule cutoff. PM-7: Patient security negative matrix (HTTP + unit + mobile state). PM-8: Layer A `PatientMobileMvpE2eTests` 12/12 + Layer B Android runtime. No medical notes / staff ops / confirm / check-in.
...
- `GET` / `PATCH /api/v1/organization/settings` — organization profile settings (`organization_profile.read` / `organization_profile.update`); optimistic concurrency via `ExpectedVersion`; contact/timezone/branding fields; limits remain read-only in the response
