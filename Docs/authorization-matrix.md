...
- **DOCTOR:** own appointments + own availability; medical notes with author + own-appointment ownership; appointment-linked patients (Model A). No clinic administration. Authoritative Web MVP scope: `Docs/mvp-doctor-scope.md` (**approved** 2026-07-25). **In code:** `doctor_dashboard.read` (DR-1); `doctor_profile.read` / `doctor_profile.update` (DR-2); availability self-manage + queue/calendar doctor filter locked to self (DR-3); appointment API ownership + `appointments.create` removed from DOCTOR (DR-4); Model A patient search/detail (DR-5); medical-note ownership + appointment-detail UX (DR-6); clinical workflow / completion hardening (DR-7); cross-role negative matrix (DR-9). Doctor reports skipped (DR-8).
