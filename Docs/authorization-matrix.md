...
- **PATIENT:** own profile/appointments + availability read + clinic register + authenticated clinic browse + mobile booking (`Requested`) + My Appointments cancel/reschedule (2-hour cutoff). Authoritative Mobile MVP: `Docs/mvp-patient-scope.md` (**approved** 2026-07-25; **PM-0…PM-7 delivered**; **PM-8 Layer A** API pack delivered; Layer B Android runtime pending). PM-1: foreign patient profile `404`; 2-hour cancel/reschedule cutoff. PM-7: Patient security negative matrix (HTTP + unit + mobile state). PM-8: Layer A `PatientMobileMvpE2eTests`; Layer B checklist. No medical notes / staff ops / confirm / check-in.
