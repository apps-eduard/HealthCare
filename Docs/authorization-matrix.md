...
- **PATIENT:** own profile/appointments + availability read + clinic register; no staff ops. Authoritative Mobile MVP: `Docs/mvp-patient-scope.md` (**approved** 2026-07-25; **PM-0 + PM-1 delivered**; PM-2…PM-8 pending). PM-1: foreign patient profile `404`; 2-hour cancel/reschedule cutoff (`appointment.patient_mutation_cutoff` / 409). Remaining: authenticated clinic browse (PM-4); mobile (PM-2+). No medical notes / staff ops / confirm / check-in.
