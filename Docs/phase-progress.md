...
| In progress | 0 |
| Not started | 3 |
| Weighted score | (3×1.0) + (0.7 + 0.95 + 0.5 + 0.75 + 0.85 + 0.7 + 0.85 + 0.85) adjusted with Phase 11 at ~40% ≈ **67%** |

**Scoring rule:** Complete = 100% of phase · Partial = 50% (or noted fraction) · In progress = 25% · Not started / Blocked = 0%

**Current focus:** Patient Mobile MVP **PM-8 complete** (Layer A + Layer B). Do not start a post-PM-8 milestone until scheduled.
...
| 11 | Patient mobile application | Partial | `███████░░░` 70% |
...
- **Staff availability management:** `/availability` (Doctor Availability). Nav when `availability.read` or any of `availability.manage_self` / `availability.manage_clinic` / `availability.manage_organization`. Mutations require manage. Clinic/doctor selection: doctor self fixed; clinic admin fixed clinic + doctor picker; org admin `ClinicPicker` + `IClinicWorkingContext` then Guid doctors via `GET .../staff/clinics/{id}/doctors`; PLATFORM_ADMIN requires org + clinic bypass. Weekly windows (Mon–Sun), create/edit/delete with `ExpectedVersion` + optional `clinicId`, date exceptions create/delete (no exception edit), Effective Availability client composition (max 14 days), clinic timezone label (`ClinicTimeZoneId`), available-slots preview. Typed `IDoctorAvailabilityApiClient`. Helpers in `HealthCare.Web.Availability`.
...
**Status:** **Complete for approved Patient Mobile MVP** — **PM-0…PM-8 delivered** (Layer A + Layer B Android runtime, 2026-07-26)  
...
- Patient API journey E2E pack on docsvr (`PatientMobileMvpE2eTests` 12/12) — PM-8 Layer A
- Android emulator runtime acceptance (`HealthCare_Pixel_API34`, API 34) — PM-8 Layer B
- Focused unit + HTTP integration coverage

### Remaining (post-MVP)

- Deferred items only (App Links, Google/OTP, iOS, notifications, Patient Web, clinical summaries, etc.)

Phase 11 Patient Mobile MVP DoD is met. Do not start an unscheduled post-PM-8 milestone.
...
| 2026-07-24 | 8 | Org Admin Frontend Phase 5 — doctor availability UI (windows/exceptions/effective/slots, clinic context, tests) |
...
| 2026-07-24 | 8 | Clinic Admin CA-3 — actor-aware staff management UI hardening + E2E smoke |
| 2026-07-25 | 8 | Clinic Admin CA-4 — doctor directory `/doctors` + availability actor polish + E2E smoke |
...
| 2026-07-25 | 8 | Clinic Admin CA-9 — clinic-filtered audit API/UI + `clinic_audit_logs.read` + allowlist + E2E smoke |
| 2026-07-25 | 8 | Clinic Admin CA-10 — hardening + E2E pack; Clinic Admin Web MVP complete |

### Clinic Admin Web MVP (approved scope)

Authoritative scope: **`Docs/mvp-clinic-admin-scope.md`** (product-owner approved 2026-07-24). **Status: complete (CA-1–CA-10).**
...
| CA-3 | Staff UI hardening (clinic actor) | Small | **Delivered** (2026-07-24) |
...
| 2026-07-25 | 8 | Doctor DR-2 — `doctor_profile.read/update` + Doctor Profile API/UI + My Profile nav |
...
| 2026-07-26 | — | **PM-8 Layer B:** Pixel API 34 emulator runtime acceptance; Patient Mobile MVP complete |

### Patient Mobile MVP (approved scope)

Authoritative scope: **`Docs/mvp-patient-scope.md`** (**approved** 2026-07-25). **Status: PM-0…PM-8 complete (Patient Mobile MVP).**

Patient-facing app: **.NET MAUI Blazor Hybrid** (Android first). No separate Patient Web app in the initial MVP. Existing Patient APIs retained; PM-1 closed 404 concealment + 2-hour cancel/reschedule cutoff + DTO review. PM-2–PM-6 delivered shell through My Appointments. PM-7 delivered the Patient security negative matrix (complements DR-9; representative staff-surface coverage). PM-8 Layer A delivers real-API Patient journey verification on docsvr (12/12); Layer B delivers Android emulator runtime acceptance (`HealthCare_Pixel_API34`).
...
**Remaining gaps after PM-8:** deferred features only (App Links, Google/OTP, iOS, notifications, Patient Web, clinical summaries). Confirmation App Links remain a documented PM-3 limitation.

**Do not** start a post-PM-8 milestone until scheduled.
