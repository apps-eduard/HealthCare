...
- Auth: anonymous requests to protected pages challenge to `/login?returnUrl=...` (no 500). Staff Web uses an **HttpOnly BFF cookie** (`HealthCare.Staff.Auth`) for host authentication. API access/refresh tokens are stored **server-side only** (distributed cache + data protection) and attached by `AuthDelegatingHandler`. Browser never receives API tokens. Login posts to `/bff/auth/login`; logout uses `/bff/auth/logout`.
