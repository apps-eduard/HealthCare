...
public sealed class ClinicReportQueryValidator : AbstractValidator<ClinicReportQuery>
{
    public const int MaxInclusiveDays = 93;

    public ClinicReportQueryValidator()
    {
        RuleFor(x => x.ClinicId)
            .Must(id => !id.HasValue || id.Value != Guid.Empty)
            .WithErrorCode(ClinicReportErrorCodes.ClinicNotFound)
            .WithMessage("ClinicId is invalid.");

        RuleFor(x => x.FromDate)
            .Must(d => string.IsNullOrWhiteSpace(d) || DateOnly.TryParse(d, out _))
            .WithErrorCode(ClinicReportErrorCodes.InvalidDateRange)
